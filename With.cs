...
namespace System.Immutable {
  ...
  // That is, once constructed, all public fields and getters will always return the same values
...
namespace System.Immutable {
  ...
  static class WithPrivate {
    ...
    static IEnumerable<MemberInfo> GetMembers(Type type) {
      ...
      memberList = memberList.AddRange(type.GetMembers(BindingFlags.DeclaredOnly | BindingFlags.Instance | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.Static));
      memberList = memberList.AddRange(GetMembers(type.BaseType));
